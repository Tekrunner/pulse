...
- first readable visual: ≤ 3 seconds.

The budget is intentionally a ceiling for the fixture pilot and is re-measured by the production-site story against the real report and dataset. The build is static and same-origin; no remote font or data request is part of the artifact.
...
The observed cross-browser maxima were 116 ms and 2,343 ms. The Playwright test enforces both ceilings in Chromium and Firefox on every verification run.
