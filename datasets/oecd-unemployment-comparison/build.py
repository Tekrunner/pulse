...
def build(context):
    ...
    if snapshot.format != "parquet":
        raise DatasetError(
            "oecd-unemployment-rate publishes provider-native Parquet rows", stage="transform"
        )
    run_dbt(
    ...
    try:
        rows, start, end = connection.execute(
            # `period` is a month START; the represented period ends on the last
            # day of that month, which is what the publication deadline advances
            # from.
            "SELECT count(*), min(period)::VARCHAR,"
            " last_day(max(period))::VARCHAR FROM read_parquet(?)",
