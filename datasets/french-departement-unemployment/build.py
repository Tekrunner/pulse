...
No analytical transformation lives here: territorial decoding, typing and every
data-quality rule are in package-owned dbt SQL.
"""
...
def build(context):
    ...
    if snapshot.format != "parquet":
        raise DatasetError(
            "insee-local-unemployment publishes provider-native Parquet rows", stage="transform"
        )
    run_dbt(
    ...
    return DatasetBuildResult(
        ...
        {"input_format": snapshot.format, "territory_quarters": rows},
    )
