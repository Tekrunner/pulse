...
No analytical transformation lives here: code and label decoding, membership,
the derived ratios, the labour-input screen and every data-quality rule are in
...
def build(context):
    ...
    if snapshot.format != "parquet":
        raise DatasetError(
            "oecd-productivity-database publishes provider-native Parquet rows", stage="transform"
        )
    run_dbt(
    ...
    try:
        rows, start, end = connection.execute(
            ...
            # on the last day of the latest year any area reaches, which is what
