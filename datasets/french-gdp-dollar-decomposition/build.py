...
No analytical transformation lives here: the franc-to-euro conversion, the
log decomposition and every data-quality rule are in package-owned dbt SQL.
"""
...
def build(context):
    ...
    if snapshot.format != "parquet":
        raise DatasetError(
            "world-bank-wdi-gdp publishes provider-native Parquet rows", stage="transform"
        )
    run_dbt(
    ...
    try:
        rows, start, end = connection.execute(
            ...
            # on the last day of the latest year, which is what the publication
            # deadline advances from.
    ...
    return DatasetBuildResult(
        ...
        {"input_format": snapshot.format, "years": rows},
    )
