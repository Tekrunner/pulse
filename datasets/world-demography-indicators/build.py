...
def build(context):
    ...
    try:
        rows, start, end, locations, worlds = connection.execute(
            """
            SELECT
              count(*),
              min(period)::VARCHAR,
              ...
              -- table reported as ending on 1 January would look a year older
              -- than it is, and would be called late a year early.
