# European emigration flows dataset contract
...
ceased to have usual residence in that country during the year, counted
regardless of citizenship. 855 rows across 36 reporting areas, read from the
`eurostat-emigration` snapshot.

Its sibling `european-immigration-flows` is the same table in the opposite
direction, on the same series key.

## What "regardless of citizenship" buys, and why it matters

The acquisition pins `citizen = TOTAL`, so this count includes departing
nationals, not foreign nationals only. That is what makes it differenceable
against the arrival count published on the same basis: arrivals minus departures
is a balance over the same population on both sides.
...
`emigration_persons` is nullable for the same reason. A year the provider
withholds is not a year in which nobody left.
