...
No analytical transformation lives here: the NUTS-to-departement
correspondence, the index to France and every data-quality rule are in
...
def build(context):
    ...
    if snapshot.format != "parquet":
        raise DatasetError(
            "eurostat-regional-gdp publishes provider-native Parquet rows", stage="transform"
        )
    run_dbt(
