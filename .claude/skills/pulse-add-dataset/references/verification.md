...
Inspect the emitted manifest and Parquet with DuckDB. Verify exact types, row grain, represented-period bounds, non-empty indicators, semantic metadata, snapshot hash lineage, deterministic rebuild hash, and that a deliberately invalid input or dbt test failure retains the prior manifest/Parquet pair. Encode what you inspect as rules, not as today's numbers: a test over the committed publication reads its row count, bounds and hash from `publish/public/data/<id>/dataset.json` (`AGENTS.md` › Tests over published data); only tests over recorded fixtures may assert literals. Keep live provider access outside this workflow.
