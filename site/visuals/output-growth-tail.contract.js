...
/** Rejects rows that do not match the consumer schema: an annual or quarter row with a positive volume and nullable growth. */
export function validateOutputGrowthTailRows(rows) {
  if (!Array.isArray(rows)) throw new TypeError("Visual rows must be an array.");
  return rows.map((row) => {
    if (
      !["annual", "quarter"].includes(row?.kind) || typeof row?.period !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(row.period)
      || !Number.isFinite(row?.gdp_volume_eur_bn) || row.gdp_volume_eur_bn <= 0
      || !nullableNumber(row?.growth_pct ?? null) || !nullableNumber(row?.quarter_on_quarter_pct ?? null)
      || !nullableNumber(row?.gdp_per_capita_chained_eur ?? null)
      || (row.kind === "annual" && !Number.isFinite(row.gdp_per_capita_chained_eur))
    ) {
