...
```
https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/migr_imm1ctz/A.TOTAL.COMPLET.TOTAL.NR.T.?format=SDMX-CSV
```
...
citizenship-specific one, which matters because the provider also publishes
the same flow broken down by country of citizenship.
...
`0 5 8 4,5,6 *` — 05:00 UTC on the eighth of April, May and June.
...
runs exist for the continuous revision the provider documents above: a
national statistical institute restating a year after the March release is
normal here, and two further passes catch a restatement within the season
without polling a small annual dataset all year.
