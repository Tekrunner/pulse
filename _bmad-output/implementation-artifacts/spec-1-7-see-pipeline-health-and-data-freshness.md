...
**Never:** `system/site`, `build-site`/`deploy-site` outcomes, or site-level freshness — dropped from this story on 2026-09-09 as redundant with browser-computed data freshness. No per-figure status notices, no new severity tokens, and no bespoke disclosure widget — reuse the existing `<details>` pattern; degraded state is carried by text, and `--color-error` is the only colour it may reuse. No aggregate health box: the UX memlog rejects it as redundant with the all-pipelines list. No precomputed staleness strings, GitHub Actions APIs or other mutable external state, remote assets, route-derived URLs, color-only state cues, stack traces or raw upstream text in diagnostics, and no scheduled acquisition (Story 1.8).
