...
**Never:** `system/site`, `build-site`/`deploy-site` outcomes, or site-level freshness — dropped from this story on 2026-09-09 as redundant with browser-computed data freshness. No per-figure status notices and no bespoke disclosure widget — reuse the existing `<details>` pattern. Severity colour is additive only: every state must still be carried by its marker glyph and state word, and each severity colour must clear WCAG AA on `--color-panel` and stay off the `--color-focus` hue, which is reserved for focus rings. No aggregate health box: the UX memlog rejects it as redundant with the all-pipelines list. No precomputed staleness strings, GitHub Actions APIs or other mutable external state, remote assets, route-derived URLs, color-only state cues, stack traces or raw upstream text in diagnostics, and no scheduled acquisition (Story 1.8).
...
- 2026-09-09: Human review of the built homepage found two deviations from settled UX decisions in `ux-designs/ux-pulse-2026-08-11/.memlog.md`: health was not verifiable at a glance (line 15 makes that the surface's primary outcome) and every pipeline rendered flat rather than compact-by-default with expandable detail (lines 17, 26, 37, 38). Root cause: planning loaded the compiled `epic-1-context.md`, which had dropped these decisions, and step-01 forbids reading raw planning docs; a later simplification pass then wrote "text in exactly two places" and "no new status component" into Boundaries, entrenching the flat list. Amended Always to require a compact row (name, status, execution date) plus a per-pipeline disclosure and at-a-glance scanning, and Never to permit the existing `<details>` pattern while still banning an aggregate health box (memlog line 58 rejects it as redundant). KEEP: the two-surface limit, text-not-colour state, `--color-error` as the only severity colour, no per-figure notices, browser-side staleness derivation, and the report's single provenance qualification line — all correct, and must survive re-derivation.
- 2026-09-09: Human asked for colour so health states pop at a glance, and reaffirmed it after being shown that `epics.md:738` requires "healthy state remains quiet". Recorded as the human's decision: success now renders green rather than muted, which is a deliberate deviation from that clause. Amended Never to permit severity tokens under explicit conditions (additive to glyph and word, WCAG AA on `--color-panel`, off the reserved `--color-focus` hue) instead of banning them outright. Added `--color-ok` (#6ee7a0, 10.67:1) and `--color-warn` (#f2994a, 7.40:1); failed and suspect/stale no longer share one red edge, which had made them indistinguishable. KEEP: marker glyph plus state word on every row, so no state is ever carried by colour alone.

- 2026-09-09: Human asked how pipelines were ordered. They were sorted alphabetically by `pipelineId`, which was never a decision: the `dataset:`/`source:` prefix put every dataset above its own source, so a failing source rendered below the datasets it had broken, and a degraded row could sit anywhere in a longer list. The UX memlog settles report sorting (line 20) but never pipeline sorting. Human chose a hybrid: status first, then sources before datasets. `not-run` ranks above healthy because whether a pipeline executed is the first question, and below the degraded states because those carry evidence to act on.
