"""Explicit, opt-in provider check for the INSEE annual national-accounts source.
...
OPT_IN = "PULSE_LIVE_INSEE_ANNUAL_NATIONAL_ACCOUNTS"


@pytest.mark.live
@pytest.mark.skipif(os.environ.get(OPT_IN) != "1", reason=f"{OPT_IN} is not set to 1")
def test_declared_dataflows_still_answer_with_a_current_release() -> None:
    declaration = discover_sources()["insee-annual-national-accounts"]
