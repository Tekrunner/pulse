"""Explicit, opt-in provider check for the OECD Productivity Database source.

Excluded from ordinary verification by the `live` marker and skipped unless the
source-specific opt-in variable is set, so no routine run can reach the OECD.
"""
...
OPT_IN = "PULSE_LIVE_OECD_PRODUCTIVITY_DATABASE"
...
def test_declared_scope_still_matches_the_live_provider_response() -> None:
    declaration = discover_sources()["oecd-productivity-database"]
