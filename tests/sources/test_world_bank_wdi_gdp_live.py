"""Explicit, opt-in provider check for the World Bank WDI GDP source.

Excluded from ordinary verification by the `live` marker and skipped unless the
source-specific opt-in variable is set, so no routine run can reach the World Bank.
"""
...
OPT_IN = "PULSE_LIVE_WORLD_BANK_WDI_GDP"
...
def test_declared_scope_still_matches_the_live_provider_response() -> None:
    declaration = discover_sources()["world-bank-wdi-gdp"]
