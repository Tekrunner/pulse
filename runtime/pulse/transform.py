...
def _snapshot(archive_root: Path) -> tuple[Path, Any]:
...
def _write_landing(raw: Path, snapshot: Any, root: Path) -> tuple[Path, LandingManifest]:
    ...
    try:
        ...
        try:
            ...
            con.close()
        additions = sorted(set(observed) - REQUIRED - {"DECIMALS", "OBS_STATUS", "OBS_QUAL", "DATE_JO", "LAST_UPDATE", "OBS_TYPE", "TITLE_EN", "UNIT_MEASURE"})
...
def _validate_insee_landing(connection: duckdb.DuckDBPyConnection, raw: Path) -> None:
    ...
    invalid = connection.execute("""
        ...
        WHERE IDBANK NOT IN ('011814056', '011814057', '011814058')
           OR FREQ != 'M' OR REF_AREA != 'FE' OR UNIT_MULT != '0'
    """, [str(raw)]).fetchone()[0]
    ...
    expected_titles = {series_id: values[1] for series_id, values in SERIES.items()}
    observed_titles = {series_id: minimum for series_id, minimum, maximum in titles if minimum == maximum}
    if invalid or duplicates or observed_titles != expected_titles:
        raise TransformError("landing does not match the declared INSEE series contract")


def _dbt(project: Path, landing: Path, output: Path, warehouse: Path) -> None:
    ...
    run = subprocess.run(
        [executable, "build", "--select", "+insee_cpi_monthly", "--vars", variables],
        cwd=project,
        env=env,
        text=True,
        capture_output=True,
...
def replay_insee(*, archive_root: Path = ROOT / "snapshots/public", landing_root: Path = ROOT / "build/landing/public/insee-cpi", publish_root: Path = ROOT / "publish/public/data/insee-cpi/monthly") -> DatasetManifest:
    ...
    try:
        snapshot_path, snapshot = _snapshot(archive_root)
